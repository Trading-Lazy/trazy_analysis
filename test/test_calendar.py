...
def test_euronext_calendar_holiday_mayday():
    ...
    expected_df = pd.DataFrame(expected_df_business_cal,
                               columns=['timestamp', 'market_open', 'market_close'])
    expected_df['timestamp'] = pd.to_datetime(expected_df.timestamp, format=DATE_FORMAT)
    expected_df['market_open'] = pd.to_datetime(expected_df['market_open'], format="%Y-%m-%d %H:%M:%S%z")
    expected_df['market_close'] = pd.to_datetime(expected_df['market_close'], format="%Y-%m-%d %H:%M:%S%z")
