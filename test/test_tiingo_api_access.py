...
def test_generate_ticker_url():
    ...
    TiingoApiAccess.API_TOKEN = "abcde"
    expected_url = (
    ...
    assert expected_url == TiingoApiAccess.generate_ticker_url(ticker, period)
...
def test_parse_get_tickers_response():
    ...
    assert (
        TiingoApiAccess.parse_get_tickers_response(tickers_response)
...
def test_ticker_data_is_none():
    ...
    assert TiingoApiAccess.ticker_data_is_none(data)
...
def test_ticker_data_to_dataframe():
    data = (
        ...
        "2020-06-17 09:31:00-04:00,354.28,354.96,353.96,354.78,2324.0\n"
        "2020-06-17 09:32:00-04:00,354.92,355.32,354.09,354.09,1123.0\n"
        "2020-06-17 09:33:00-04:00,354.25,354.59,354.14,354.59,2613.0\n"
        "2020-06-17 09:34:00-04:00,354.22,354.26,353.95,353.98,1186.0\n"
    )
    df = TiingoApiAccess.ticker_data_to_dataframe(data)
    ...
    expected_df_columns_values = {
        ...
        "volume": [3254.0, 2324.0, 1123.0, 2613.0, 1186.0],
    }
