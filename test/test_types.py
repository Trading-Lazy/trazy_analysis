...
def test_rescale():
    ...
    assert aggregated_candle_dataframe.get_candle(0) == Candle(
        ...
        close=323.81,
        volume=500,
        timestamp=datetime.strptime("2020-05-07 14:20:00+00:00", "%Y-%m-%d %H:%M:%S%z"),
    )
    ...
    assert aggregated_candle_dataframe.asset == Asset(symbol=SYMBOL, exchange="IEX", time_unit=timedelta(minutes=5))
