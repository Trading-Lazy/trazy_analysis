...
SYMBOL = "IVV"
CANDLE1 = Candle(asset=Asset(symbol=SYMBOL, exchange="IEX"), open=323.69, high=323.81, low=323.67, close=323.81,
                 volume=500, timestamp=datetime.strptime("2020-05-07 14:24:00+0000", "%Y-%m-%d %H:%M:%S%z"))
CANDLE2 = Candle(asset=Asset(symbol=SYMBOL, exchange="IEX"), open=323.81, high=324.21, low=323.81, close=324.1,
                 volume=700, timestamp=datetime.strptime("2020-05-07 14:25:00+0000", "%Y-%m-%d %H:%M:%S%z"))
CANDLE3 = Candle(asset=Asset(symbol=SYMBOL, exchange="IEX"), open=324.1, high=324.1, low=323.97, close=324.03,
                 volume=400, timestamp=datetime.strptime("2020-05-07 14:26:00+0000", "%Y-%m-%d %H:%M:%S%z"))
CANDLE4 = Candle(asset=Asset(symbol=SYMBOL, exchange="IEX"), open=323.93, high=323.95, low=323.83, close=323.88,
...
def test_candle_dataframe():
    candle_dataframe = CandleDataFrame.from_candle_list(asset=Asset(symbol=SYMBOL, exchange="IEX"),
                                                        candles=np.array([CANDLE1, CANDLE2, CANDLE3, CANDLE4],
                                                                         dtype=Candle))
...
def test_candle_dataframe_duplicate_index_in_init():
    with pytest.raises(ValueError):
        CandleDataFrame.from_candle_list(Asset(symbol=SYMBOL, exchange="IEX"), candles=np.array(
            [CANDLE1, CANDLE2, CANDLE3, CANDLE4, CANDLE4], dtype=Candle
        ))


def test_candle_dataframe_add_candle():
    candle_dataframe = CandleDataFrame.from_candle_list(Asset(symbol=SYMBOL, exchange="IEX"),
...
def test_append():
    candle_dataframe1 = CandleDataFrame.from_candle_list(Asset(symbol=SYMBOL, exchange="IEX"),
                                                         candles=np.array([CANDLE1, CANDLE2], dtype=Candle))
    candle_dataframe2 = CandleDataFrame.from_candle_list(Asset(symbol=SYMBOL, exchange="IEX"),
...
def test_from_candle_list():
    ...
    candle_dataframe = CandleDataFrame.from_candle_list(asset=Asset(SYMBOL, "IEX"), candles=candles)
...
def test_concat():
    ...
    concatenated_candle_dataframe = CandleDataFrame.concat([candle_dataframe1, candle_dataframe2, candle_dataframe3],
...
def test_concat_duplicate_index():
    ...
    with pytest.raises(Exception):
        CandleDataFrame.concat([candle_dataframe1, candle_dataframe2, candle_dataframe3], SYMBOL)
...
def test_rescale():
    candle_dataframe = CandleDataFrame.from_candle_list(asset=Asset(symbol=SYMBOL, exchange="IEX"),
                                                        candles=np.array([CANDLE1, CANDLE2, CANDLE3, CANDLE4],
                                                                         dtype=Candle))
    aggregated_candle_dataframe = candle_dataframe.rescale(
        timedelta(minutes=5), MARKET_CAL, remove_incomplete_head=False
    )
    assert len(aggregated_candle_dataframe) == 3
    assert aggregated_candle_dataframe.get_candle(0) == Candle(asset=Asset(symbol="IVV", exchange="IEX"), open=323.69,
                                                               high=323.81, low=323.67, close=323.81, volume=500,
                                                               timestamp=datetime.strptime("2020-05-07 14:20:00+00:00",
                                                                                           "%Y-%m-%d %H:%M:%S%z"))
    assert aggregated_candle_dataframe.get_candle(1) == Candle(asset=Asset(symbol="IVV", exchange="IEX"), open=323.81,
                                                               high=324.21, low=323.81, close=324.03, volume=1100,
                                                               timestamp=datetime.strptime("2020-05-07 14:25:00+00:00",
                                                                                           "%Y-%m-%d %H:%M:%S%z"))
    assert aggregated_candle_dataframe.get_candle(2) == Candle(asset=Asset(symbol="IVV", exchange="IEX"), open=323.93,
                                                               high=323.95, low=323.83, close=323.88, volume=300,
                                                               timestamp=datetime.strptime("2020-05-07 14:30:00+00:00",
                                                                                           "%Y-%m-%d %H:%M:%S%z"))
    assert aggregated_candle_dataframe.asset == Asset(symbol=SYMBOL, exchange="IEX")


def test_rescale_empty_candle_dataframe():
    candle_dataframe = CandleDataFrame.from_candle_list(Asset(symbol=SYMBOL, exchange="IEX"),
                                                        candles=np.array([], dtype=Candle))
    aggregated_candle_dataframe = candle_dataframe.rescale(
