...
class HistoricalDataHandler(DataHandler, metaclass=RateLimitedSingletonMeta):
    ...
    def request_ticker_data_for_period(
        ...
        if response:
            if not cls.ticker_data_is_none(data):
                ...
                LOG.info(
                    "No available data for ticker {} for period {}".format(
                        ticker, period
                    )
                )
                none_response_periods.add(period)
        else:
            LOG.info(
                "Ticker {} request error for period {}: status_code = {}, message = {}".format(
                    ticker, period, response.status_code, data
                )
            )
    ...
    def request_ticker_data_from_periods(
        cls, ticker: str, periods: List[Tuple[pd.Timestamp, pd.Timestamp]]
