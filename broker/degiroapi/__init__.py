...
class DeGiro:
    ...
    def orders(self, from_date, to_date, not_executed=None):
        ...
        data = self.__request(
            ...
            None,
            orders_payload,
            error_message="Could not get orders.",
        )["data"]
        data_not_executed = []
        if not_executed:
            for d in data:
    ...
    def delete_order(self, orderId):
        ...
        return self.__request(
            DeGiro.__ORDER_URL + orderId + ";jsessionid=" + self.session_id,
            None,
            delete_order_params,
    ...
    def filtercashfunds(cashfunds):
        ...
        for item in cashfunds["cashFunds"]["value"]:
            if item["value"][2]["value"] != 0:
                data.append(
                    item["value"][1]["value"] + " " + str(item["value"][2]["value"])
                )
        return data
    ...
    def filterportfolio(portfolio, filter_zero=None):
        ...
        for item in portfolio["portfolio"]["value"]:
            ...
            for i in item["value"]:
                ...
                positionType = (
                    i["value"] if i["name"] == "positionType" else positionType
                )
                price = i["value"] if i["name"] == "price" else price
                value = i["value"] if i["name"] == "value" else value
                breakEvenPrice = (
                    i["value"] if i["name"] == "breakEvenPrice" else breakEvenPrice
                )
            data.append(
                {
                    "id": item["id"],
    ...
    def real_time_price(self, product_id, interval):
        ...
        return self.__request(
            DeGiro.__PRICE_DATA_URL,
            None,
            price_payload,
            error_message="Could not get real time price",
        )["series"]
    ...
    def buyorder(
        ...
        if (
            orderType != Order.Type.STOPLIMIT
            and orderType != Order.Type.MARKET
            and orderType != Order.Type.LIMIT
            and orderType != Order.Type.STOPLOSS
        ):
    ...
    def get_stock_list(self, indexId, stockCountryId):
        ...
        return self.__request(
            DeGiro.__GET_STOCKS_URL,
            None,
            stock_list_params,
            error_message="Could not get stock list",
        )["products"]
