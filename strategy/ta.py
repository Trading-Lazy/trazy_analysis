...
def ma(df: pd.DataFrame, n):
    sma = pd.Series(df["close"].rolling(n).mean(), name="MA_" + str(n))
    df = df.join(sma)
    return df
...
def EMA(df, n):
    EMA = pd.Series(
        pd.ewma(df["Close"], span=n, min_periods=n - 1), name="EMA_" + str(n)
    )
    df = df.join(EMA)
    return df
...
def ADX(df, n, n_ADX):
    ...
    ADX = pd.Series(
        pd.ewma(
            abs(PosDI - NegDI) / (PosDI + NegDI), span=n_ADX, min_periods=n_ADX - 1
        ),
...
def Chaikin(df):
    ...
    Chaikin = pd.Series(
        pd.ewma(ad, span=3, min_periods=2) - pd.ewma(ad, span=10, min_periods=9),
        name="Chaikin",
    )
    df = df.join(Chaikin)
    return df
...
def CCI(df, n):
    PP = (df["High"] + df["Low"] + df["Close"]) / 3
    CCI = pd.Series(
        (PP - pd.rolling_mean(PP, n)) / pd.rolling_std(PP, n), name="CCI_" + str(n)
    )
    df = df.join(CCI)
    return df
...
def ULTOSC(df):
    ...
    UltO = pd.Series(
        ...
        + (
            2
            * pd.rolling_sum(pd.Series(BP_l), 14)
            / pd.rolling_sum(pd.Series(TR_l), 14)
        )
