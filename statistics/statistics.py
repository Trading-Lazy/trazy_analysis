...
import trazy_analysis.statistics.performance as perf
...
class Statistics(AbstractStatistics):
    ...
    def get_tearsheet(self) -> pd.DataFrame:
        ...
        """
        stats = self.get_results(equity=self.equity, returns=self.returns)
        bench_stats = self.get_results(
            equity=self.benchmark_equity, returns=self.benchmark_returns
        )
        ...
        if bench_stats is not None:
            ...
            tearsheet = pd.DataFrame(
                data=[
                    [tot_ret, bench_tot_ret],
                    [cagr, bench_cagr],
                    [sharpe, bench_sharpe],
                    [sortino, bench_sortino],
                    [
                        returns.std() * np.sqrt(self.periods),
                        bench_returns.std() * np.sqrt(self.periods),
                    ],
                    [dd_max, bench_dd_max],
                    [dd_dur, bench_dd_dur],
                ],
                index=index,
                columns=["Backtest results", "Benchmark results"],
            )
            return tearsheet
    ...
    def get_results(self, equity: pd.DataFrame, returns: pd.Series) -> dict:
        """
        ...
        dd_s, max_dd, dd_dur = perf.create_drawdowns(cum_returns)

        # Equity statistics
        statistics = {}
        statistics["sharpe"] = perf.create_sharpe_ratio(returns, self.periods)
        ...
        statistics["equity"] = equity["Equity"]
        statistics["returns"] = returns
        statistics["cum_returns"] = cum_returns
        return statistics
